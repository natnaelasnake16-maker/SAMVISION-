...
const Shop: React.FC<ShopProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="bg-[#fbfcfd] min-h-screen">
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 lg:p-12 overflow-y-auto">
          ...
          <div className="relative w-full max-w-4xl bg-white border border-white/20 shadow-[0_100px_100px_-50px_rgba(0,0,0,0.5)] animate-fade-up rounded-[2rem] md:rounded-[3rem] overflow-hidden flex flex-col md:flex-row max-h-[90vh] md:max-h-[85vh]">
            <button
              onClick={() => setSelectedProduct(null)}
              className="absolute top-4 right-4 md:top-8 md:right-8 z-50 w-10 h-10 rounded-full glass border-slate-200/60 hover:bg-slate-50 flex items-center justify-center transition-all group"
            >
              <svg className="w-5 h-5 text-slate-500 group-hover:rotate-90 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>

            <div className="md:w-[45%] bg-slate-50/80 p-6 md:p-12 flex items-center justify-center relative overflow-hidden">
              ...
              {selectedProduct.image ? (
                <img src={selectedProduct.image} className="w-full h-64 md:h-auto object-contain drop-shadow-[0_10px_30px_rgba(0,0,0,0.1)] relative z-10 transition-transform duration-700 hover:scale-105" alt={selectedProduct.name || 'Frame'} />
              ) : (
            ...
            <div className="md:w-[55%] p-8 md:p-12 lg:p-14 space-y-8 md:space-y-10 overflow-y-auto bg-white">
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <p className="text-[9px] font-black text-primary uppercase tracking-[0.4em] opacity-80">{selectedProduct.brand}</p>
                  {selectedProduct.homepageFlags?.isNew && <span className="text-[7px] font-black px-2 py-0.5 bg-primary/10 text-primary rounded-full uppercase tracking-widest border border-primary/10">New Arrival</span>}
                </div>
                <h2 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight leading-tight">{selectedProduct.name}</h2>
                <p className="text-xs md:text-sm text-slate-500 font-medium leading-relaxed max-w-sm">Optimized for ZEISS precision lenses. Part of the limited {selectedProduct.collection} series.</p>
              </div>

              <div className="space-y-1.5">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Price Points</p>
                <div className="flex items-baseline space-x-3">
                  <span className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter leading-none">{selectedProduct.price.toLocaleString()} <span className="text-xs text-slate-400 uppercase tracking-widest ml-1 font-bold">ETB</span></span>
                  {selectedProduct.originalPrice && <span className="text-lg text-slate-300 line-through font-bold opacity-70">{selectedProduct.originalPrice.toLocaleString()}</span>}
                </div>
              </div>

              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-4 bg-slate-50/80 border border-slate-100 rounded-2xl space-y-0.5">
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Material & Frame</p>
                    <p className="text-xs font-bold text-slate-800 tracking-tight">{selectedProduct.material} • {selectedProduct.rimType}</p>
                  </div>
                  <div className="p-4 bg-slate-50/80 border border-slate-100 rounded-2xl space-y-0.5">
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Lens Optics</p>
                    <p className="text-xs font-bold text-slate-800 tracking-tight">ZEISS Compatible</p>
                  </div>
                </div>
              </div>

              <div className="space-y-4">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">In-Store Inventory</p>
                <div className="grid gap-2">
                  {Object.entries(selectedProduct.stockPerBranch || {})
                    ...
                    .map(([branchId]) => {
                      ...
                      return branch ? (
                        <div key={branchId} className="flex items-center justify-between p-3.5 bg-slate-50/50 rounded-xl border border-slate-100/50">
                          <div className="flex items-center space-x-3">
                            <div className="w-1.5 h-1.5 rounded-full bg-teal-400" />
                            <span className="text-[11px] font-bold text-slate-700">{branch.name}</span>
                          </div>
                  ...
                  {(!selectedProduct.stockPerBranch || Object.values(selectedProduct.stockPerBranch).every(s => (s as any).quantity === 0)) && (
                    <div className="p-3.5 bg-orange-50/50 border border-orange-100 rounded-xl flex items-center space-x-2 text-orange-600">
                      ...
                      <span className="text-[9px] font-bold uppercase tracking-widest">Request Stock Check</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3 pt-2">
                <a
                  ...
                  className="bg-primary text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-lg shadow-teal-500/10 transition-all transform hover:translate-y-[-2px] active:translate-y-0 flex items-center justify-center space-x-2"
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                  <span>Inquire Now</span>
                </a>
                <button
                  onClick={() => window.open('https://wa.me/251941545454', '_blank')}
                  className="bg-slate-900 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl hover:bg-black transition-all transform hover:translate-y-[-2px] active:translate-y-0 flex items-center justify-center space-x-2"
                >
                  <img src="https://www.svgrepo.com/show/475692/whatsapp-color.svg" className="w-4 h-4" alt="WhatsApp" />
